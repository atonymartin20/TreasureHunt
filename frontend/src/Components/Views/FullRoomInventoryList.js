...
class FullRoomInventoryList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            userData: {},
    ...
    render() {
        ...
        if(Object.keys(this.context.state.currentRoomData).length !== 0) {
            ...
            if (this.props.ListType === "exits") {
                if (Object.keys(this.context.state.currentRoomData.exits).length > 0) {
                    return (
                        <ul>
                            {this.context.state.currentRoomData.exits.map(exit => {
            ...
            // Checks ListType being passed through props
            if (this.props.ListType === "messages") {
                if ((this.context.state.currentRoomData.messages).length > 1) {
                    return (
                        <ul>
            ...
            // Checks ListType being passed through props
            if (this.props.ListType === "errors") {
                if ((this.context.state.currentRoomData.errors).length > 1) {
                    return (
                        <ul>
                            {this.context.state.currentRoomData.errors.map(error => {
                                return <li key={Math.random()}>{error}</li>
                            })}
                        </ul>
                    );
                }
                else if ((this.context.state.currentRoomData.errors).length === 1) {
                    let errorSpan = ''
                    this.context.state.currentRoomData.errors.map(error => {
            ...
            // If the prop ListType doesn't match "players", "items", "exits, ""messages", or "errors" return null.
            else {
                return null
            }
        }
...
FullRoomInventoryList.contextType = AppContext;

export default FullRoomInventoryList;